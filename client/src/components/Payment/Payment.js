...
const Payment = () => {
	...
	useEffect(() => {
		const getAmount = () => {
			const filterData = context.selectSeats.filter((ele) => ele.isReserved === true && ele.name === user);
		...
		getAmount();
	}, [context.selectSeats, user]);

	const paymentClickHandler = async () => {
		try {
			await axios.post(`http://localhost:8000/api/seats/insertmany`, context.selectSeats);
			alert('Payment successfull');
			context.setSelectSeats([]);
			const res = await axios.get('http://localhost:8000/api/seats');
			...
			<Container>
				<Row>
					<Col md={{ span: 6, offset: 3 }} className={style.main}>
						<h5 className="pb-5 paymt-header">Booking details</h5>
						<Row>
							<Col sm={9}>
